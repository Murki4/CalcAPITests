...
public class PostDivide {
    ...
    public void PostDivide() {
        ...
        Assertions.assertEquals(Double.parseDouble(request_body.getNumber_1())
                / Double.parseDouble(request_body.getNumber_2()), result.getResult());
    }
    ...
    public void PostDivideZero() {
        PostRequestBody request_body = new PostRequestBody("35", "0", "/");
        ResultData result = given()
                .spec(Specifications.authCred())
                .body(request_body)
                .when()
